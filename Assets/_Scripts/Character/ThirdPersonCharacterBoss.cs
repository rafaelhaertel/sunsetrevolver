...
namespace UnityStandardAssets.Characters.ThirdPerson2
{
	...
	public class ThirdPersonCharacterBoss : MonoBehaviour
	{
		...
		void HandleAirborneMovement(bool ChangePlat, Vector3 moveRaw)
		{
			...
			if (Mathf.Abs (m_Rigidbody.velocity.z) < 4) {
				moveRaw = new Vector3 (moveRaw.x, moveRaw.y, moveRaw.z * airpush);
...
//			}
			...
			if (ChangePlat) {
				m_Rigidbody.velocity = new Vector3 (-6f, m_Rigidbody.velocity.y + 2f, m_Rigidbody.velocity.z);
